class Room {
    ...
    // 统计公开投票
    count_public_vote() {
        let result = this.players.map(player => `${player.name} : ${player.public_vote}`).join('<br>');
        for (let player of this.players) {
            player.public_vote = null;
        }
        this.public_voted = 0;
        return 'Open Vote Results: <br>' + result;
    }
    ...
    count_private_vote() {
        ...
        for (let player of this.players) {
            if (player.private_vote === "Success") {
                vote_counts.success += 1;
            } else if (player.private_vote === "Fail") {
                vote_counts.fail += 1;
            } else if (player.private_vote === "Not in Mission") {
                vote_counts.not_in_mission += 1;
            }
        }

        let result = `Anolymous Vote Results: <br> success: ${vote_counts.success} <br> fail: ${vote_counts.fail} <br> not in mission: ${vote_counts.not_in_mission}`;

        for (let player of this.players) {
            player.private_vote = null;
        }
        this.private_voted = 0;
...
class Player {
    constructor(name, password, id, role = null) {
        ...
        this.socketId = id;
        this.role = role;
        this.public_vote = null;
        this.private_vote = null;
    }
    ...
    //公开投票
    public_voter(content, room) {
        if (this.public_vote === null) {
            room.public_voted += 1;
            this.public_vote = content;
        } else {
            this.public_vote = content;
        }
    }
    ...
    //匿名投票
    private_voter(content, room) {
        if (this.private_vote === null) {
            room.private_voted += 1;
            this.private_vote = content;
        } else {
            this.private_vote = content;
        }
    }
